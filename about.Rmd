...
initial_date <- as.Date("2025-04-24")
today <- Sys.Date()
source(here("keys.R"))
slackr_setup(token = token)

# Function to format market cap in millions with 1 decimal and "M" suffix
...
#   stock_data$date <- Sys.Date()
#   # Convert to data frame and save to CSV
#   write_csv(stock_data, "nasdaq_screener.csv")
#   cat("✅ Data saved to 'nasdaq_screener.csv'\n")
# } else {
#   cat("❌ Failed to fetch data. Status code:", status_code(response), "\n")
# }

today_stock_data <- read_csv(here("nasdaq_screener.csv")) |>
  filter(nchar(symbol)<5) |> # filter out stocks with more than 4 characters (not tradeable)
  select(-name, -url, -netchange, -pctchange) |> # remove unnecessary columns
...
cummulative_stock_data <- readRDS(here("cummulative_stock_data.RDS")) |>
...
new_data <- anti_join(today_stock_data, 
                      cummulative_stock_data, 
                      by = c("symbol", "date"))
clean_cummulative_stock_data <- bind_rows(cummulative_stock_data, new_data) |>
  arrange(symbol, date)

# Fill NA values backward
stock_data_filled <- clean_cummulative_stock_data |>
  group_by(symbol) |>
  arrange(date, .by_group = TRUE) |>
  mutate(lastsale = na.locf(lastsale, fromLast = TRUE)) |>
  ungroup() |>
  arrange(symbol, date)
...
stocks_data_grow <- stock_data_filled |>
  group_by(symbol) |>
  arrange(date, .by_group = TRUE) |>
...
stock_data_pct_change <- stocks_data_grow |>
  ...
  mutate(daily_pct_change = (lastsale - lag(lastsale)) / lag(lastsale) * 100,
         pct_change = (lastsale - first(lastsale)) / first(lastsale) * 100) |>
  filter((max(head(pct_change, -1)) / 2) <= last(pct_change)) |> # max{first, ..., last-1} / 2 <= last
  ungroup() 
...
symbols_with_negative_pct_change <- stock_data_pct_change |>
  group_by(symbol) |>
  arrange(date, .by_group = TRUE) |>
  filter(any(pct_change < 0)) |> # this filter is so that the next make sense
  filter(abs(min(pct_change)) > last(pct_change)) |>
  ungroup() |>
  distinct(symbol) |>
  pull(symbol)
...
info_of_surviving_symbols <- aux |>
  ...
  select(-date, -lastsale, -daily_pct_change, pct_change) |>
  mutate(marketCap_formatted = format_market_cap(marketCap))
...
aug_acc_rank <- acc_rank |>
  ...
  arrange(desc(acc_rank)) |>
  select(symbol, acc_rank, everything())

aug_sum_rank <- sum_rank |>
  inner_join(info_of_surviving_symbols, by = "symbol") |>
  arrange(desc(sum_rank)) |>
  select(symbol, sum_rank, everything())

aug_last_day_rank <- last_day_rank |>
  inner_join(info_of_surviving_symbols, by = "symbol") |>
  arrange(desc(last_rank)) |>
  select(symbol, last_rank, everything())
```
