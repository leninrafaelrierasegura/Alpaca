...
Column `entrynumver` is added to keep track of the number of entries for each symbol. It is there to avoid removing rows that were NA-backfilled.
...
#   write_csv(stock_data, here("data/nasdaq_screener.csv"))
#   cat("✅ Data saved to 'data/nasdaq_screener.csv'\n")
# } else {
...
clean_cummulative_stock_data <- bind_rows(cummulative_stock_data, today_stock_data) |>
  distinct(across(-date), .keep_all = TRUE) |> # remove duplicates
  arrange(symbol, date) |>
