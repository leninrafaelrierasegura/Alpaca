...
aug_acc_rank <- acc_rank |>
  ...
  arrange(desc(acc_rank)) |>
  select(symbol, acc_rank, marketCap, marketCap_, accum_pct_change, country, industry, sector, ipoyear, everything()) |>
  mutate(link = paste0("https://finance.yahoo.com/quote/", symbol))

aug_sum_rank <- sum_rank |>
  inner_join(info_of_surviving_symbols, by = "symbol") |>
  arrange(desc(sum_rank)) |>
  select(symbol, sum_rank, marketCap, marketCap_, accum_pct_change, country, industry, sector, ipoyear, everything()) |>
  mutate(link = paste0("https://finance.yahoo.com/quote/", symbol))

aug_last_day_rank <- last_day_rank |>
  inner_join(info_of_surviving_symbols, by = "symbol") |>
  arrange(desc(last_rank)) |>
  select(symbol, last_rank, marketCap, marketCap_, accum_pct_change, country, industry, sector, ipoyear, everything()) |>
  mutate(link = paste0("https://finance.yahoo.com/quote/", symbol))
```
...
datatable(
  ...
  options = list(
    ...
    initComplete = JS(
      "function(settings) {",
      "  var api = this.api();",
      "  // Create a new header row for the filters",
